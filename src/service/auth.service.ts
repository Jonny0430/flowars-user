...
export const AuthService = {
	...
	async checkAgent(token?: string) {
		try {
			const { data } = await axios.get(`${API_URL}${getAuthUrl('check-agent')}`, {
				headers: {
	...
	async checkAdmin(token?: string) {
		try {
			const { data } = await axios.get(`${API_URL}${getAuthUrl('check-admin')}`, {
				headers: {
					Authorization: `Bearer ${token}`
				}
			})
	...
	async checkAllUser(token?: string) {
		try {
			const { data } = await axios.get(`${API_URL}${getAuthUrl('checkAllUser')}`, {
				headers: {
					Authorization: `Bearer ${token}`
				}
			})
			...
			console.log('CheckAllUser check failed:', error)
		}
	},
	...
	async getMyProducts() {
		try {
			const { data } = await $axios.get(`${getUserUrl('my-products')}`)
