            ...
            >
                <HStack py={6} px={5} spacing={4} bg={useColorModeValue('gray.100', 'gray.800')} w="100%">
                    ...
                    <VStack spacing={0} align="start" maxW="lg" h="100%">
                        <Text as="h3" fontSize="md" noOfLines={2} color={useColorModeValue('', '')}>
                            {t(data.label, { ns: 'home' })}
                        </Text>
                        <HStack spacing={2}>
                ...
                <Flex
                    ...
                    alignItems="center"
                    px={5}
                    _groupHover={{ visibility: 'visible', opacity: 1, height: '60px' }}
                    transition="opacity 0.3s ease-in-out, visibility 0.3s ease-in-out, height 0.3s ease-in-out"
                >
                    <Button variant={'outline'} colorScheme='facebook' onClick={() => router.push('/statistics')} rightIcon={<TbCircleArrowUpRight />} w={'250px'}>
                        {t('hero_stats_4', { ns: 'home' })}
                    </Button>
                </Flex>
            </Stack>
        </motion.div>
