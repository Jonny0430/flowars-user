...
const Curriculum = () => {
    ...
    const hover = useColorModeValue('facebook.600', 'facebook.300')

    return (
        <>
            <Heading mt={10}>{t('curriculum', { ns: 'products' })}</Heading>
            <Flex align={'center'} gap={2} mt={3}>
                {data.length} {t('modules', { ns: 'products' })}{'  '}
                <Icon as={CiCircleQuestion} />{'  '}
                {data.map(c => c.lessons.length).reduce((a, b) => +a + +b, 0)}{'  '}
                {t('lessons', { ns: 'products' })}
            </Flex>
